...
class Kinematics:
    ...
    def process_linear_move(self,xyzuvw_In):
        """
        Parses a motion command, applies rounding logic,
        plans waypoints, computes IK, and drives motors.
        """

        # -----------------------------
        # Direction flags for each axis
        # -----------------------------
        self.J1dir = self.J2dir = self.J3dir = 0
        self.J4dir = self.J5dir = self.J6dir = 0
        self.J7dir = self.J8dir = self.J9dir = 0
        
        # Current delay between motor steps (microseconds)
        self.curDelay = 0.0

        # Axis fault flags
        self.J1axisFault = self.J2axisFault = self.J3axisFault = 0
        self.J4axisFault = self.J5axisFault = self.J6axisFault = 0
        self.J7axisFault = self.J8axisFault = self.J9axisFault = 0
        self.TotalAxisFault = 0

        # -----------------------------
        # Parse command string indices
        # -----------------------------
        # End points
        xStart = self.outgoingaxyzuvew[0]
        yStart = self.outgoingxyzuvw.find("Y")
        zStart = self.outgoingxyzuvw.find("Z")
        rzStart = self.outgoingxyzuvw[5]
        ryStart = self.outgoingxyzuvw.find("Ry")
        rxStart = self.outgoingxyzuvw.find("Rx")
        SPstart = inData.find("S")
        AcStart = inData.find("Ac")
        DcStart = inData.find("Dc")
        RmStart = inData.find("Rm")
        RndStart = inData.find("Rnd")
        WristConStart = inData.find("W")
        LoopModeStart = inData.find("Lm")
        DisWristStart = inData.find("Q")

        # -----------------------------
        # Parse target Cartesian pose
        # -----------------------------
        self.xyzuvw_Temp[0] = float(inData[xStart + 1:yStart])
        self.xyzuvw_Temp[1] = float(inData[yStart + 1:zStart])
        self.xyzuvw_Temp[2] = float(inData[zStart + 1:rzStart])
        self.xyzuvw_Temp[3] = float(inData[rzStart + 2:ryStart])
        self.xyzuvw_Temp[4] = float(inData[ryStart + 2:rxStart])
        self.xyzuvw_Temp[5] = float(inData[rxStart + 2:J7Start])

        # External axes
        self.J7_In = float(inData[J7Start + 2:J8Start])
        self.J8_In = float(inData[J8Start + 2:J9Start])
        self.J9_In = float(inData[J9Start + 2:SPstart])

        # -----------------------------
        # Speed and motion parameters
        # -----------------------------
        SpeedType = inData[SPstart + 1:SPstart + 2]
        SpeedVal = float(inData[SPstart + 2:AcStart])
        ACCspd = float(inData[AcStart + 2:DcStart])
        DCCspd = float(inData[DcStart + 2:RmStart])
        ACCramp = float(inData[RmStart + 2:RndStart])
        Rounding = float(inData[RndStart + 3:WristConStart])

        self.WristCon = inData[WristConStart + 1:LoopModeStart]
        LoopMode = inData[LoopModeStart + 2:DisWristStart]
        self.DisWrist = inData[DisWristStart + 1:].strip()

        # Loop mode per joint
        self.J1LoopMode = int(LoopMode[0])
        self.J2LoopMode = int(LoopMode[1])
        self.J3LoopMode = int(LoopMode[2])
        self.J4LoopMode = int(LoopMode[3])
        self.J5LoopMode = int(LoopMode[4])
        self.J6LoopMode = int(LoopMode[5:])

        # ---------------------------------------------------------
        # Update current position & handle rounding (spline logic)
        # ---------------------------------------------------------
        self.updatePos()

        # Linear motion vector
        Xvect = self.xyzuvw_Temp[0] - self.xyzuvw_Out[0]
        Yvect = self.xyzuvw_Temp[1] - self.xyzuvw_Out[1]
        Zvect = self.xyzuvw_Temp[2] - self.xyzuvw_Out[2]
        RZvect = self.xyzuvw_Temp[3] - self.xyzuvw_Out[3]
        RYvect = self.xyzuvw_Temp[4] - self.xyzuvw_Out[4]
        RXvect = self.xyzuvw_Temp[5] - self.xyzuvw_Out[5]

        # Distance of motion in 6D space
        lineDist = math.sqrt(
            Xvect**2 + Yvect**2 + Zvect**2 +
            RZvect**2 + RYvect**2 + RXvect**2
        )
        ...
        # -----------------------------------------
        wayPts = lineDist / self.linWayDistSP
        wayPerc = 1.0 / wayPts

        # -----------------------------------------
        # Pre-calculate destination joint steps
        # -----------------------------------------
        self.SolveInverseKinematics()

        Jfut = [
            int((self.JangleOut[i] + getattr(self, f"J{i+1}axisLimNeg")) *
                getattr(self, f"J{i+1}StepDeg"))
            for i in range(6)
        ]

        JstepDif = [
            getattr(self, f"J{i+1}StepM") - Jfut[i]
            for i in range(6)
        ]

        # Highest step delta governs timing
        HighStep = max(abs(s) for s in JstepDif)
        ...
        if SpeedType == "s":
            self.speedSP = SpeedVal * 1_000_000 * 1.2
        elif SpeedType == "m":
            self.speedSP = (lineDist / SpeedVal) * 1_000_000 * 1.2
        else:  # percentage
            self.speedSP = self.minSpeedDelay / (SpeedVal / 100)

        calcStepGap = max(self.minSpeedDelay, self.speedSP / HighStep)
        ...
        for i in range(int(wayPts) + 2):

            curWayPerc = wayPerc * i

            # Interpolate Cartesian pose
            for j in range(6):
                self.xyzuvw_In[j] = (
                    self.xyzuvw_Out[j] +
                    (self.xyzuvw_Temp[j] - self.xyzuvw_Out[j]) * curWayPerc
                )

            self.SolveInverseKinematics()

            # Compute new step deltas
            Jfut = [
                int((self.JangleOut[j] + getattr(self, f"J{j+1}axisLimNeg")) *
                    getattr(self, f"J{j+1}StepDeg"))
                for j in range(6)
            ]

            JstepDif = [
                getattr(self, f"J{j+1}StepM") - Jfut[j]
                for j in range(6)
            ]

            # Direction flags
            Jdirs = [1 if d <= 0 else 0 for d in JstepDif]
            ...
            faults = []
            for j in range(6):
                stepM = getattr(self, f"J{j+1}StepM")
                stepLim = getattr(self, f"J{j+1}StepLim")
                if (Jdirs[j] == 1 and stepM + JstepDif[j] > stepLim) or \
                   (Jdirs[j] == 0 and stepM - JstepDif[j] < 0):
                    faults.append(1)
                else:
                    faults.append(0)

            self.TotalAxisFault = sum(faults)

            # -----------------------------------------
            # Execute motor move
            # -----------------------------------------
            if self.TotalAxisFault == 0 and self.KinematicError == 0:
                self.driveMotorsL(
                    *(abs(d) for d in JstepDif),
                    abs(self.J7stepDif), abs(self.J8stepDif), abs(self.J9stepDif),
                    *Jdirs, self.J7dir, self.J8dir, self.J9dir,
                    self.curDelay
                )
                self.updatePos()

            elif self.KinematicError:
                self.Alarm = "ER"
                print(self.Alarm)
                time.sleep(0.005)

            else:
                self.Alarm = "EL" + "".join(str(f) for f in faults)
                print(self.Alarm)
                time.sleep(0.005)
